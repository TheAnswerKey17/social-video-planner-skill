...
在把文章、字幕或素材改写成 `script.md` 之前，必须先完成
`content-brief.md`：

- 读懂素材并总结核心问题 / 核心结论。
- 提炼 strong points、good lines、evidence、tension。
- 给出 2-3 个平台化内容角度，并明确推荐方向。
- 通过 Discussion Checkpoint 确认平台、目标观众、角度、时长、口吻和
  查证项。

`script.md` 必须服从 `content-brief.md` 的 `Script Instructions`。如果用户
没有回复但授权继续，要按 brief 的推荐方向写，并在后续
`content-review.md` 标记这些默认假设。
...
### 5. 在同一次工作里进入审核、video outline 与 handoff 草案

`script.md` 落盘 + 自检通过后，**不要立即停下来等用户**。继续完成：

1. 按 [`CONTENT-REVIEW.md`](CONTENT-REVIEW.md) 生成 `content-review.md`
   （claim/source/material/blocker 审核）。
2. 按 [`OUTLINE-FORMAT.md`](OUTLINE-FORMAT.md) 生成 `outline.md`
   （基于最终脚本的 video outline：beat / scene 规划）。
3. 按 [`HYPERFRAMES-HANDOFF.md`](HYPERFRAMES-HANDOFF.md) 生成
   `hyperframes-handoff.md`。

然后再进入 checkpoint，让用户一次确认稿件、事实来源、素材缺口和是否
可以交给 HyperFrames。
